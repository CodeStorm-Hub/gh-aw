...
3. Compile the workflow markdown into a YAML workflow file using:
   ...
   This will generate a workflow YAML lock file i`.github/workflows/<workflow-name>.lock.yml` based on the content of your markdown file.

4. Commit and push the workflow file and its lock file to your repository.

If you haven't done so already, [set up your repository secrets](/gh-aw/reference/engines/) for the coding agent your workflow will be using.
...
## Creating Agentic Workflows by adding an existing workflow

You create agentic workflows by adding any existing workflow you have access to.
...
3. Use the `gh aw add` command to add the workflow to your repository:
   ...
   ```bash wrap
   gh aw add https://github.com/githubnext/agentics/blob/main/workflows/daily-repo-status.md
   ```

   This will work for any URL to a workflow markdown file on GitHub. You should check carefully the workflow comes from a trusted source.
